...
import { InteractConfig } from './interact-config.model';
 ...
 */
export const warn = (msg: string, enableInteraction?: boolean, type?: RhInteractType) => {
  console.warn(msg);
  if (enableInteraction && typeof RH_INTERACT_CONFIG?.warnMsg === 'function') {
    ...
    if (type === 'modal' && typeof RH_INTERACT_CONFIG?.warnModal === 'function') {
...
/** 控制台打印`error`信息 */
export const error = (msg: string, enableInteraction?: boolean, type?: RhInteractType) => {
  console.error(msg);
  if (enableInteraction && typeof RH_INTERACT_CONFIG?.errorMsg === 'function') {
    ...
    if (type === 'modal' && typeof RH_INTERACT_CONFIG?.errorModal === 'function') {
...
/** 控制台打印`log`信息 */
export const log = (msg: string, enableInteraction?: boolean, type?: RhInteractType) => {
  console.log(msg);
  if (enableInteraction) {
    ...
    if (type === 'modal' && typeof RH_INTERACT_CONFIG?.logModal === 'function') {
