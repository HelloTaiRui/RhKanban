...
import { RhmNumberHelper } from "./number-helper";
import * as _ from 'lodash';

export class RhmArrayHelper {

    static sum(numberArr: number[] = []) {
        return numberArr.reduce((total, cur) => RhmNumberHelper.add(total, cur), 0);
    }

    static sumBy(objArr: number[] = [], iteratee: string | ((value: any) => number)) {
        let fn = typeof iteratee == "string" ? (obj) => { return obj[iteratee] } : iteratee;
        return objArr.reduce((total, cur) => RhmNumberHelper.add(total, fn(cur)), 0);
    }
...
export class ArrayHelper {
   ...
   */
  public static setSelectedItemStatus<T extends Record<string, RhSafeAny>, K extends keyof T>(
    list: T[],
    item: T,
    ptyName: K
  ): void {
  ...
  public static getPageIndexOfTable<T, K extends keyof T>(
    ...
    let pageIndex = 1;
    const index = dataset.findIndex((item) => (item[propertyName] as RhSafeAny) === value);
    if (index !== -1) {
   ...
   */
  public static uniqWith<T>(array: Array<T>, comparator: (object: T, other: T) => boolean): Array<T> {
   ...
   */
  public static differenceBy<T>(source: Array<T>, target: Array<T>, propertyName: string) {
