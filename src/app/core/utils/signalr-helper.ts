...
export class RhmSignalrHelper {
  ...
  initConnection(
    ...
    if (this._proxy) {
      //console.log(`signalr初始化成功！！`,this._proxy,ipAddress,hubProxyName,method);
      //绑定客户端方法，这里注意名称首字母小写
      this._proxy.on(method, (data: any) => {
        //console.log("第一手数据",data,fn);
        fn(data);
      });
    }
  ...
  stopConenction() {
    ...
    if (this.connection) {
      ...
      //this.tools.msgHelper.showToast(`SignalR已断开！！`);
    }
  }
      ...
      if (this.isDisconnect) {
        if (this.retryTime === 0) {
          ...
          setTimeout(() => {
            ...
            /* this.tools.msgHelper.showToast(
              `SignalR连接断开!!! 系统已尝试自动重启5次，仍未连接到SignalR，请刷新页面或联系管理员`
            ); */
          }, 5000);
        }
